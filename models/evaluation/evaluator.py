...
def inference_on_dataset(
    ...
    with ExitStack() as stack:
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            # df = pd.DataFrame.from_dict(video_miou, orient="index")
            # df.to_csv(output_dir + "/vis/video_miou.csv", header=False)
...
def inference_on_dataset_ss(
    ...
    with ExitStack() as stack:
        ...
        for idx, inputs in enumerate(data_loader):
            ...
            #         # video_name = "/".join(video_name)
            #     # pred_img_path = (output_dir + "/vis" + file_names[num_img % len(outputs)].split("AVSBench_semantic")[1]).replace(".jpg", ".png")
            #     pred_img_path = (output_dir + "/binary_mask" + file_names[num_img % len(outputs)].split("AVSBench_semantic")[1]).replace(".jpg", ".png")
            #     pred_img_path = pred_img_path.replace("/processed_frames/", "/")
            #     dir_name = os.path.dirname(pred_img_path)
            #     os.makedirs(dir_name, exist_ok=True)
            #     label_rgb = np.zeros((pred.shape[0], pred.shape[1], 3), dtype=np.uint8)
