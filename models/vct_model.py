...
from .modeling.criterion_s4_pac import SetCriterion_S4_PAC
from .modeling.criterion_ss_pac import SetCriterion_SS_PAC
...
class VCT(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        use_pre_sam = cfg.MODEL.PRE_SAM.USE_PRE_SAM # True
        pre_sam_dim = cfg.MODEL.PRE_SAM.PRE_SAM_DIM # [256, 512, 1024, 2048]

        if use_pre_sam: # 是否使用 Siam-Encoder Module
            #* separate backbone
        ...
        if cfg.MODEL.AUDIO.FREEZE_AUDIO_EXTRACTOR: # True
        ...
        is_avss_data = True if cfg.INPUT.DATASET_MAPPER_NAME == "avss_semantic" else False

        sem_seg_head = build_sem_seg_head(cfg, backbone.output_shape())

        # fusion module for early fusion
        if cfg.MODEL.FUSE_CONFIG.FUSION_STEP == "early": # False
        ...
        proto_weight = cfg.MODEL.MASK_FORMER.PROTO_WEIGHT
        cosine_weight = cfg.MODEL.MASK_FORMER.COSINE_WEIGHT # 0.0
        # building criterion
        ...
        if cfg.INPUT.DATASET_MAPPER_NAME == "avss_semantic":
            criterion = SetCriterion_SS_PAC(
                sem_seg_head.num_classes, # 71 or 141
                matcher=matcher, # Matcher HungarianMatcher
                weight_dict=weight_dict,
                eos_coef=no_object_weight, # 0.1
                losses=losses, # ['labels', 'masks']
        ...
        elif cfg.INPUT.DATASET_MAPPER_NAME == "avss4_semantic":
            criterion = SetCriterion_S4_PAC(
        ...
        return {
            ...
            "criterion": criterion,
            "is_s4_data": is_s4_data,
            "is_ms3_data": is_ms3_data,
    ...
    def forward(self, batched_inputs):
        ...
        for x in batched_inputs:
            ...
            )  # * [bs*5, 1, 96, 64] # [B, 1, 96, 64]
            if self.is_s4_data:
                category_labels.append(x["category_label"])
        images = [(x - self.pixel_mean) / self.pixel_std for x in images]
        images = ImageList.from_tensors(images, self.size_divisibility) # image_sizes(B个(224, 224)) 和 tensor([B, 3, 224, 224])

        with torch.no_grad():
            audio_feature = self.audio_backbone(audio_log_mels)  # * [bs*5, 128] # [video_num*10, 128] # 修改后:[video_num*10, 512, 6, 4]
        # audio_feature = audio_feature.unsqueeze(1)
        if self.is_avss_data:
            audio_feature = audio_feature[vid_temporal_mask_flag.bool()] # [B, 512, 6, 4] (v1s and v1m *5)(v2 *10)
        ...
        if self.early_fusion:
            ...
            outputs = self.sem_seg_head(features, audio_feature)  # dict {pred_logits, pred_masks, aux_outputs} # 修改后:audio_feature[B, 512, 6, 4]
        if self.training:
            ...
            elif self.is_s4_data:
                losses = self.criterion(outputs, targets, category_labels)
            else:
            ...
            for num_img, (mask_cls_result, mask_pred_result, image_size) in enumerate(
                ...
                # For fair comparison with other 224x224 methods
                height = 224
                width = 224
    ...
    def semantic_inference_ss(self, mask_cls, mask_pred, vid_temporal_mask_flag):
        mask_cls = F.softmax(mask_cls, dim=-1)[..., :-1] # [100, 142] -> [100, 141]
        mask_pred = mask_pred.sigmoid() # [100, 224, 224]
        semseg = torch.einsum("qc,qhw->chw", mask_cls, mask_pred)
