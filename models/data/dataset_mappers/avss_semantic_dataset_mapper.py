...
def resize_img(crop_size, img, is_mask=False):
    ...
    if not is_mask:
        img = Image.fromarray(img).resize((outsize, outsize), Image.BILINEAR)
    else:
        img = Image.fromarray(img).resize((outsize, outsize), Image.NEAREST)
    return np.array(img)
...
class AVSS_SemanticDatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        for image_path in dataset_dict["file_names"]:
            ...
            if self.is_train:
                img_short_size = min(image.shape[0], image.shape[1])
                image = resize_img(img_short_size, image, is_mask=False)
            images.append(image)
        ...
        if "sem_seg_file_names" in dataset_dict:
            ...
            for sem_seg_file_name in sem_seg_file_name_list:
                ...
                if self.is_train:
                    gt_short_size = min(sem_seg_gt.shape[0], sem_seg_gt.shape[1])
                    sem_seg_gt = resize_img(gt_short_size, sem_seg_gt, is_mask=True)
        ...
        if ("pre_mask_file_names" in dataset_dict) and self.pre_sam:
            ...
            for pre_mask_file_name in pre_mask_file_name_list:
                ...
                if self.is_train:
                    pre_short_size = min(pre_mask_gt.shape[0], pre_mask_gt.shape[1])
                    pre_mask_gt = resize_img(pre_short_size, pre_mask_gt, is_mask=True)
