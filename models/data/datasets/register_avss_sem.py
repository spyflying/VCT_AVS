...
def _get_avss_files(df_split, root, split, pre_mask_dir=None):
    ...
    for index in np.arange(0, len(df_split)):
        ...
        if split != 'train': # val and test
            img_base_path = os.path.join(root, set, video_name, "processed_frames_384")  # frames -> processed_frames
            pre_mask_path = os.path.join(pre_mask_dir, set, video_name, "processed_frames_384") if pre_mask_dir else None  # frames -> processed_frames
            mask_base_path = os.path.join(root, set, video_name, "processed_labels_semantic")  # labels_semantic  -> processed_labels_semantic
        else: # train
            img_base_path = os.path.join(root, set, video_name, "frames")  # frames -> processed_frames
            pre_mask_path = os.path.join(pre_mask_dir, set, video_name, "frames") if pre_mask_dir else None  # frames -> processed_frames
            mask_base_path = os.path.join(root, set, video_name, "labels_semantic")  # labels_semantic  -> processed_labels_semantic
        ...
        if set == "v1s":
            ...
            if split == "train":
                gt_temporal_mask_flag = [1, 0, 0, 0, 0]
            else:
                gt_temporal_mask_flag = [1, 1, 1, 1, 1]
