...
class MSDeformAttnPixelDecoder(nn.Module):
        ...
        if self.transformer_num_feature_levels > 1:
            ...
            for in_channels in transformer_in_channels[::-1]:
                input_proj_list.append(
                    nn.Sequential(
                        nn.Conv2d(in_channels, conv_dim, kernel_size=1), # conv_dim = 256
    ...
    def forward_features(self, features):
        ...
        for idx, f in enumerate(self.transformer_in_features[::-1]):  # * ['res3', 'res4', 'res5']
            x = features[f].float()  # deformable detr does not support half precision [B, 256, 7, 7], [B, 512, 14, 14], [B, 1024, 28, 28]
            srcs.append(self.input_proj[idx](x)) # [B, 256, 7, 7], [B, 256, 14, 14], [B, 256, 28, 28]
            pos.append(self.pe_layer(x)) # [B, 256, 7, 7], [B, 256, 14, 14], [B, 256, 28, 28]

        y, spatial_shapes, level_start_index = self.transformer(srcs, pos)
        bs = y.shape[0]

        split_size_or_sections = [None] * self.transformer_num_feature_levels
        for i in range(self.transformer_num_feature_levels): # 3
        ...
        for o in out: # [B, 256, 7, 7], [B, 256, 14, 14], [B, 256, 28, 28], [B, 256, 56, 56]
