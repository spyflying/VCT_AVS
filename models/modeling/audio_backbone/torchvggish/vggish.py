...
class VGG(nn.Module):
    def __init__(self, features):
        ...
        self.embeddings = nn.Sequential(
            nn.Linear(512 * 4 * 6, 4096), nn.ReLU(True), nn.Linear(4096, 4096), nn.ReLU(True), nn.Linear(4096, 128), nn.ReLU(True)
        )

    def forward(self, x, compact=False): # [B, 1, 96, 64]
        x = self.features(x) # [B, 512, 6, 4]

        # Transpose the output from features to
        # remain compatible with vggish embeddings
        if compact:
            x = torch.transpose(x, 1, 3) # [40, 4, 6, 512]
            x = torch.transpose(x, 1, 2) # [40, 6, 4, 512]
            x = x.contiguous()
            x = x.view(x.size(0), -1) # [40, 12288]

            return self.embeddings(x) # [40, 128]
...
class VGGish(VGG):
    def __init__(self, cfg, device=None):
        ...
        if cfg.MODEL.AUDIO.FREEZE_AUDIO_EXTRACTOR:
            state_dict = torch.load(cfg.MODEL.AUDIO.PRETRAINED_VGGISH_MODEL_PATH)
            super().load_state_dict(state_dict)
            print(f"==> Load pretrained VGGish parameters from {cfg.MODEL.AUDIO.PRETRAINED_VGGISH_MODEL_PATH}")
        ...
        self.preprocess = cfg.MODEL.AUDIO.PREPROCESS_AUDIO_TO_LOG_MEL
        self.postprocess = cfg.MODEL.AUDIO.POSTPROCESS_LOG_MEL_WITH_PCA
        if self.postprocess:
            self.pproc = Postprocessor()
            if cfg.MODEL.AUDIO.FREEZE_AUDIO_EXTRACTOR:
                state_dict = torch.load(cfg.MODEL.AUDIO.PRETRAINED_PCA_PARAMS_PATH)
        ...
        self.compact = cfg.MODEL.AUDIO.COMPACT_FEATURE
        # self.to(self.device)

    def forward(self, x): # [40, 1, 96, 64]
        if self.preprocess: # False
            print(">>> pre processing...")
            x = self._preprocess(x)
            x = x.to(self.device)
        x = VGG.forward(self, x, self.compact) # [40, 128]
