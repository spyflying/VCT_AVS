...
class SetCriterion_SS_PAC(nn.Module):
    ...
    def prototype_loss(self, feats, targets, gt_temporal_mask_flag_index):
        audio_feat = feats['audio_feat'].unsqueeze(1) # [B, C] -> [B, 1, C]
        audio_feat = torch.index_select(audio_feat, dim=0, index=gt_temporal_mask_flag_index)
        proto_feat = feats['proto_feat'] # [B, k, C]
        proto_feat = torch.index_select(proto_feat, dim=0, index=gt_temporal_mask_flag_index)
        targets_indicator = torch.zeros(audio_feat.size(0), self.num_classes).to(audio_feat.device)
    ...
    def forward(self, outputs, targets, vid_temporal_mask_flag, gt_temporal_mask_flag):
        ...
        if 'feats' in outputs:
            l_dict = self.prototype_loss(outputs['feats'], targets, gt_temporal_mask_flag_index)
            losses.update(l_dict)
